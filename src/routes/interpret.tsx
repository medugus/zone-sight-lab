...
function InterpretPage() {
  ...
  const operatingMode = currentPlate?.operatingMode ?? "standalone";
  const interpretationAuthority = currentPlate?.interpretationAuthority ?? "measurement_only";

  return (
    <div>
      <PageHeader
        title="EUCAST Interpretation"
        description="Prepare interpretation table using validated EUCAST breakpoints."
      />
          ...
          <CardContent className="space-y-3">
            {rejected && (
              <p className="text-sm text-destructive">
                Interpretation blocked because QC is rejected.
              </p>
            )}
            <p className="text-sm">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>S breakpoint</TableHead>
                  <TableHead>R breakpoint</TableHead>
              ...
              <TableBody>
                {measurements.map((m) => (
                  <TableRow key={m.id}>
                    ...
                    <TableCell>—</TableCell>
                    <TableCell>—</TableCell>
                    <TableCell>Not interpreted</TableCell>
                    <TableCell>No breakpoint loaded</TableCell>
                    <TableCell>—</TableCell>
                  </TableRow>
                ))}
              </TableBody>
